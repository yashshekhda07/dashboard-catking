...
class CEOController extends Controller {
    ...
    public function index(Request $request) {
        if ($request->ajax()) {
            ...
            $selectedStartDate = Carbon::parse($this->startDate)->format('F jS, Y');
            $selectedEndDate = Carbon::parse($this->endDate)->format('F jS, Y');
...
//                 ->whereBetween('StartTime', [$this->startDate, $this->endDate])
        ...
        $night_time_of_calls = DB::table('ozonetel_webhook')
                ...
                ->where(function ($query) {
                    $query
                    ->whereTime('StartTime', '>=', '21:00:00')
                    ->orWhereTime('StartTime', '<=', '09:00:00');
                })
                ->count();
        // $day_time_of_calls = DB::table('ozonetel_webhook')
        // ->where('Status', 'Answered')
    ...
    public function perAgentConversion(Request $request) {
        ...
        $filter_type = isset($request->date) && !empty($request->date) ? $request->date : "";
        $agent_filter_type = isset($request->agentdate) && !empty($request->agentdate) ? $request->agentdate : "";

//        if (!empty($filter_type) && $filter_type != 'custom')

        $this->getStartEndDate($agent_filter_type);
        ...
        $agentOrderJoinRecord = DB::table('npf_webhooks')->select('npf_webhooks.owner as owner', DB::raw('count(orders.id) as converted'))->join('orders', 'orders.bd_phone_number', 'npf_webhooks.mobile')->whereNotNull('npf_webhooks.owner')->where('npf_webhooks.stage', 'Joined')->whereBetween('npf_webhooks.urd', [$this->startDate, $this->endDate])->whereBetween('orders.created_at', [$this->startDate, $this->endDate])->groupBy('npf_webhooks.owner')->pluck('converted', 'owner')->toArray();

        $agentOrderRecord = DB::table('npf_webhooks')->select('owner', DB::raw('count(stage) as leads'))->whereNotNull('owner')->groupBy('npf_webhooks.owner')->whereBetween('urd', [$this->startDate, $this->endDate])->pluck('leads', 'owner')->toArray();
        ...
        foreach ($npfRecord as $npf) {
            if (array_key_exists($npf->owner, $orderJoinRecord)) {
                $npf->converted = isset($orderJoinRecord[$npf->owner]) ? $orderJoinRecord[$npf->owner] : 0;
            } else {
                $npf->converted = 0;
            }

            if (array_key_exists($npf->owner, $agentOrderJoinRecord)) {
                $npf->previous_converted = isset($agentOrderJoinRecord[$npf->owner]) ? $agentOrderJoinRecord[$npf->owner] : 0;
            } else {
                $npf->previous_converted = 0;
            }

            if (array_key_exists($npf->owner, $agentOrderRecord)) {
                $npf->previous_leads = isset($agentOrderRecord[$npf->owner]) ? $agentOrderRecord[$npf->owner] : 0;
            } else {
                $npf->previous_leads = 0;
            }
        }
        // echo "<pre>";
        // print_r( $data);
        ...
        foreach ($npfRecord as $key => $npf_record) {
            ...
            if (!empty($agent_filter_type) && $agent_filter_type != 'custom') {
                $previous_untouched_lead_counter = DB::table('npf_webhooks')
                        ->where('owner', $npf_record->owner)
                        ->where('stage', 'Untouched')
                        ->whereBetween('urd', [$this->startDate, $this->endDate])
                        ->count();
            } else {
                $previous_untouched_lead_counter = 0;
            }

            if (!empty($agent_filter_type) && $agent_filter_type != 'custom') {
                $previous_joined_lead_counter = DB::table('npf_webhooks')
                        ->where('owner', $npf_record->owner)
                        ->where('stage', 'Untouched')
                        ->whereBetween('urd', [$this->startDate, $this->endDate])
                        ->count();
            } else {
                $previous_joined_lead_counter = 0;
            }
            ...
            $npf_record->previous_untouched_lead_counter = $previous_untouched_lead_counter;
            $npf_record->previous_untouched_lead_revenue = numberFormat($previous_untouched_lead_counter * 15000);

            $npf_record->converted_revenue = numberFormat($npf_record->converted * 15000);
            $npf_record->convertion_percentage = (($npf_record->leads) ? round(($npf_record->converted * 100) / $npf_record->leads) : 0 ) . "%";
            $npf_record->loss_revenue = numberFormat(($npf_record->leads - $npf_record->converted) * 15000);

            $npf_record->previous_convertion_percentage = (($npf_record->previous_leads) ? round(($npf_record->previous_converted * 100) / $npf_record->previous_leads) : 0 ) . "%";
            $npf_record->previous_converted_revenue = numberFormat($npf_record->previous_converted * 15000);

            $npf_record->previous_loss_revenue = numberFormat(($npf_record->previous_leads - $npf_record->previous_converted) * 15000);
        }
        ...
        return DataTables::of($npfRecord)
                        ...
                        ->editColumn('previous_untouched_lead_counter', function ($data) {
                            return '<span class="text-warning">' . $data->previous_untouched_lead_counter . '</span>';
                        })
                        ...
                        ->rawColumns(['other_leads', 'talk_time', 'progress', 'untouched', 'previous_untouched_lead_counter', 'previous_untouched_lead_revenue', 'converted_revenue'])
                        ->addIndexColumn()
                        ->toJson();
    }
