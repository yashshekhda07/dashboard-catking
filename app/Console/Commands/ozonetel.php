...
class ozonetel extends Command {
     ...
     * @var string
     */
    protected $signature = 'command:ozonetel';
     ...
     * @var string
     */
    protected $description = 'ozonetel';
    ...
    public function handle() {
        ...
        $ozonetel_last_record = DB::table('ozonetel_webhook')->latest()->first();

        if (isset($ozonetel_last_record) && !empty($ozonetel_last_record)) {
            $from_date = urlencode($ozonetel_last_record->created_at);
            $to_date = urlencode(date('Y-m-d H:i:s'));
        } else {
            $from_date = urlencode(date('Y-m-d 00:00:01'));
            $to_date = urlencode(date('Y-m-d H:i:s'));
        }
        ...
        $format = 'json';
        $url = 'https://in1-ccaas-api.ozonetel.com/cloudAgentRestAPI/index.php/CloudAgent/CloudAgentAPI/getCDR?api_key=' . $api_key . '&user_name=' . $user_name . '&from_date=' . $from_date . '&to_date=' . $to_date . '&format=' . $format;
